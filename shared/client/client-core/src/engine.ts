...
import {wait, settle} from './utils.js';
...
const searchForTerm = (term: string, pagedTo = ''): APIInvokerFunction => async (): Promise<APIResponse> => {
...
const issueSearch = async (apiCall: APIInvokerFunction): Promise<APIResponse> => {
...
const readyToIssueSearch = async (term: string, setResults: ResultsReceiverFunction, pagedTo = '') => {
